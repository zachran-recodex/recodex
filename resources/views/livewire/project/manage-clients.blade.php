<flux:container class="space-y-4 sm:space-y-6">
    <div class="flex flex-col space-y-4 sm:space-y-6 md:space-y-0 md:flex-row md:items-center md:justify-between">
        <div class="w-full md:w-auto">
            <flux:heading size="xl" class="font-bold! text-center md:text-left">Manage Clients</flux:heading>
        </div>
        ...
        <div class="w-full md:w-auto">
            <flux:breadcrumbs class="justify-center md:justify-start">
                ...
                <flux:breadcrumbs.item>Manage Clients</flux:breadcrumbs.item>
            </flux:breadcrumbs>
        </div>
    </div>
    ...
    <flux:card>
        <flux:card.header>
            <div class="flex flex-col sm:flex-row gap-4 sm:gap-0 items-center justify-between">
                <flux:heading size="lg" class="font-semibold! text-center sm:text-left">List Clients</flux:heading>
        ...
        <flux:card.body :padding="false">
            <div class="overflow-x-auto">
                <flux:table hover striped>
                    <flux:table.columns>
                        <flux:table.column class="min-w-[150px] md:w-auto">Logo</flux:table.column>
                        <flux:table.column class="min-w-[300px] md:w-auto">Client</flux:table.column>
                        <flux:table.column class="min-w-[200px] md:w-auto">Email</flux:table.column>
                        <flux:table.column class="min-w-[200px] md:w-auto">Phone</flux:table.column>
                    ...
                    <flux:table.rows>
                        @forelse ($clients as $client)
                            <flux:table.row>
                                ...
                                <flux:table.cell>
                                    <flux:heading class="font-semibold text-sm md:text-base">{{ $client->name }}</flux:heading>
                                </flux:table.cell>
                                <flux:table.cell class="text-sm md:text-base">{{ $client->email ?: 'N/A' }}</flux:table.cell>
                                <flux:table.cell class="text-sm md:text-base">{{ $client->phone ?: 'N/A' }}</flux:table.cell>

                                <flux:table.cell>
                                    <div class="flex gap-2">
                                        <flux:modal.trigger name="form">
                                            <flux:button variant="warning" wire:click="edit({{ $client->id }})" icon="pencil" class="!p-1.5 md:!p-2"></flux:button>
                                        </flux:modal.trigger>

                                        <flux:modal.trigger name="delete">
                                            <flux:button variant="danger" wire:click="confirmDelete({{ $client->id }})" icon="trash" class="!p-1.5 md:!p-2"></flux:button>
                                        </flux:modal.trigger>
                                    </div>
                                </flux:table.cell>
                            </flux:table.row>
                        @empty
                            <flux:table.row>
                                <flux:table.cell colspan="5" class="text-center py-6 md:py-8">
                                    ...
                                    <flux:subheading class="text-sm md:text-base">Start by creating new client.</flux:subheading>
                                </flux:table.cell>
                            </flux:table.row>
                        @endforelse
                    </flux:table.rows>
    ...
    <flux:modal name="delete" class="min-w-[280px] sm:min-w-sm">
        <div class="space-y-4 sm:space-y-6">
            <div class="text-center sm:text-left">
                <flux:heading size="lg" class="font-semibold">Delete client?</flux:heading>
            ...
            <div class="flex justify-end">
                <flux:button type="button" variant="danger" wire:click="deleteClient" class="w-full sm:w-fit">
                    Delete
                </flux:button>
            </div>
        </div>
